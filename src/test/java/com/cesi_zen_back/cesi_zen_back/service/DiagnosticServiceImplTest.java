...
class DiagnosticServiceImplTest {
  ...
  void setUp() {
    ...
    Role role = Role.builder().roleId(UUID.randomUUID()).roleName("USER").build();
  ...
  void calculateAnonymous_shouldCalculateScoreWithoutSaving() {
    ...
    when(questionRepository.findAllById(request.questionIds())).thenReturn(List.of(q1, q2));

    when(resultConfigRepository
            .findFirstByActiveTrueAndMinScoreLessThanEqualAndMaxScoreGreaterThanEqual(150, 150))
  ...
  void calculateAnonymous_shouldRejectUnknownQuestion() {
    ...
    when(questionRepository.findAllById(request.questionIds())).thenReturn(List.of(q1));
  ...
  void calculateAndSave_shouldSaveResultForAuthenticatedUser() {
    DiagnosticRequestDto request = new DiagnosticRequestDto(List.of(questionId1, questionId2));
    Jwt jwt = Jwt.withTokenValue("token").header("alg", "none").subject("user@test.fr").build();
  ...
  void createQuestion_shouldCreateActiveQuestion() {
    DiagnosticQuestionRequestDto request =
        new DiagnosticQuestionRequestDto("Nouvelle question", 42);
  ...
  void updateQuestion_shouldUpdateExistingQuestion() {
    DiagnosticQuestionRequestDto request =
        new DiagnosticQuestionRequestDto("Question modifiée", 80);

    when(questionRepository.findById(questionId1)).thenReturn(Optional.of(q1));
