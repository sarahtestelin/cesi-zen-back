...
class RessourceServiceImplTest {
  ...
  @Mock private RessourceRepository repo;

  @Mock private HistoricEtatRepository historicRepo;

  @Mock private ObjectMapper objectMapper;

  @InjectMocks private RessourceServiceImpl service;
  ...
  void create_shouldSaveRessourceAndHistory() throws Exception {
    RessourceRequestDto dto =
        new RessourceRequestDto("Nouvelle ressource", "Description", "anxiete");

    when(objectMapper.writeValueAsString(any())).thenReturn("{}");
    when(repo.save(any(Ressource.class)))
        .thenAnswer(
            invocation -> {
  ...
  void update_shouldUpdateFieldsAndSaveHistory() throws Exception {
    RessourceRequestDto dto =
        new RessourceRequestDto("Titre modifié", "Description modifiée", "sommeil");
