...
class AppUserServiceImplTest {
  ...
  @InjectMocks private AppUserServiceImpl service;
  ...
  void setUp() {
    ...
    userRole = Role.builder().roleId(UUID.randomUUID()).roleName("USER").build();
  ...
  void exportCurrentUserData_shouldReturnUserDataDiagnosticsHistoriesAndAuditLogs() {
    ...
    when(appUserRepository.findByMail("user@test.fr")).thenReturn(Optional.of(user));
    when(diagnosticResultRepository.findByAppUserIdUserOrderByCreatedAtDesc(userId))
        .thenReturn(List.of(diagnostic));
    when(historicEtatRepository.findByAppUserIdUserOrderByModificationDateDesc(userId))
        .thenReturn(List.of(history));
    when(historicEtatRepository.findByEntityTypeAndEntityIdOrderByModificationDateDesc(
            "APP_USER", userId))
        .thenReturn(List.of(history));
    when(adminAuditLogRepository.findByTargetIdOrderByCreatedAtDesc(userId.toString()))
