...
class PasswordServiceImplTest {
  ...
  void setUp() {
    ...
    ReflectionTestUtils.setField(
        passwordService, "resetPasswordUrl", "https://localhost:8443/reset-password");
  }
  ...
  void requestResetPassword_shouldCreateTokenAndSendMail_whenUserExists() {
    ...
    when(resetPasswordRepository.save(any(ResetPassword.class)))
    ...
    assertThatThrownBy(
            () ->
                passwordService.resetPassword(
                    new ResetPasswordDto("expired-token", "NewPassword123!")))
  ...
  void changePassword_shouldUpdatePassword_whenCurrentPasswordIsCorrect() {
    ...
    passwordService.changePassword(
        "user@test.fr", new ChangePasswordDto("OldPassword123!", "NewPassword123!"));
  ...
  void changePassword_shouldRejectWrongCurrentPassword() {
    ...
    assertThatThrownBy(
            () ->
                passwordService.changePassword(
                    "user@test.fr", new ChangePasswordDto("WrongPassword123!", "NewPassword123!")))
        .isInstanceOf(BadRequestException.class)
