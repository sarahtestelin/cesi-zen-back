...
class AppUserControllerTest {
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new AppUserController(appUserService))
  ...
  void getCurrentUser_shouldReturnAuthenticatedUser() throws Exception {
    AppUserDto dto =
        new AppUserDto(userId, "user@test.fr", "Sarah", true, LocalDateTime.now(), "USER");
  ...
  void updateCurrentUser_shouldCallService() throws Exception {
    AppUserDto response =
        new AppUserDto(userId, "new@test.fr", "NewSarah", true, LocalDateTime.now(), "USER");
    ...
    mockMvc
        .perform(put("/api/users/me").contentType("application/json").content(body))
        .andExpect(status().isOk())
  ...
  void updateUser_shouldForwardAdminMail() throws Exception {
    AppUserDto response =
        new AppUserDto(userId, "updated@test.fr", "Updated", true, LocalDateTime.now(), "USER");
    ...
    mockMvc
        .perform(put("/api/users/{id}", userId).contentType("application/json").content(body))
        .andExpect(status().isOk())
  ...
  void deleteUser_shouldCallServiceWithAdminMail() throws Exception {
    mockMvc.perform(delete("/api/users/{id}", userId)).andExpect(status().isOk());
  ...
  void disableUser_shouldReturnDisabledUser() throws Exception {
    AppUserDto response =
        new AppUserDto(userId, "user@test.fr", "Sarah", false, LocalDateTime.now(), "USER");

    when(appUserService.disableUser(userId, "user@test.fr")).thenReturn(response);

    mockMvc
        .perform(patch("/api/users/{id}/disable", userId))
        .andExpect(status().isOk())
  ...
  void enableUser_shouldReturnEnabledUser() throws Exception {
    ...
    mockMvc
        .perform(patch("/api/users/{id}/enable", userId))
        .andExpect(status().isOk())
