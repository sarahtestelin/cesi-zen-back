...
class RessourceControllerTest {
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new RessourceController(ressourceService))
  ...
  void listPublic_shouldReturnPublicResources() throws Exception {
    RessourceResponseDto dto =
        new RessourceResponseDto(
            ressourceId,
            true,
            true,
            "Gestion du stress",
    ...
    when(ressourceService.listPublic("stress", "sante")).thenReturn(List.of(dto));

    mockMvc
        .perform(get("/api/v1/ressources").param("search", "stress").param("category", "sante"))
  ...
  void getPublic_shouldReturnOneResource() throws Exception {
    ...
    mockMvc
        .perform(get("/api/v1/ressources/{id}", ressourceId))
        .andExpect(status().isOk())
  ...
  void create_shouldReturnCreatedAndCallService() throws Exception {
    ...
    mockMvc
        .perform(post("/api/v1/ressources").contentType("application/json").content(body))
  ...
  void update_shouldReturnUpdatedResource() throws Exception {
    ...
    when(ressourceService.update(eq(ressourceId), any(RessourceRequestDto.class)))
        .thenReturn(response);
    ...
    mockMvc
        .perform(
            put("/api/v1/ressources/{id}", ressourceId)
  ...
  void disable_shouldReturnDisabledResource() throws Exception {
    ...
    mockMvc
        .perform(patch("/api/v1/ressources/{id}/disable", ressourceId))
        .andExpect(status().isOk())
  ...
  void delete_shouldReturnNoContentAndCallService() throws Exception {
    mockMvc
        .perform(delete("/api/v1/ressources/{id}", ressourceId))
