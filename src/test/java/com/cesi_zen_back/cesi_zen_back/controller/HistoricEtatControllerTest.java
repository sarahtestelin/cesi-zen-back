...
class HistoricEtatControllerTest {
  ...
  void setUp() {
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new HistoricEtatController(historicEtatService))
  ...
  void getAllHistory_shouldReturnHistoryList() throws Exception {
    ...
    HistoricEtatResponseDto history =
        new HistoricEtatResponseDto(
            historyId, "{}", "{new}", "UPDATE", "RESSOURCE", entityId, LocalDateTime.now());

    when(historicEtatService.getAllHistory()).thenReturn(List.of(history));

    mockMvc
        .perform(get("/api/admin/history"))
  ...
  void getHistoryByEntity_shouldForwardEntityTypeAndId() throws Exception {
    ...
    HistoricEtatResponseDto history =
        new HistoricEtatResponseDto(
            historyId, "old", "new", "DISABLE", "APP_USER", entityId, LocalDateTime.now());

    when(historicEtatService.getHistoryByEntity("APP_USER", entityId)).thenReturn(List.of(history));

    mockMvc
        .perform(get("/api/admin/history/{entityType}/{entityId}", "APP_USER", entityId))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$[0].id").value(historyId.toString()))
