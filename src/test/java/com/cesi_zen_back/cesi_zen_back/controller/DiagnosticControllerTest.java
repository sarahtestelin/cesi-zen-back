...
class DiagnosticControllerTest {
  ...
  void setUp() {
    ...
    jwt = Jwt.withTokenValue("token").header("alg", "none").subject("user@test.fr").build();
    ...
    mockMvc =
        MockMvcBuilders.standaloneSetup(new DiagnosticController(diagnosticService))
  ...
  void listActiveQuestions_shouldReturnQuestions() throws Exception {
    DiagnosticQuestionResponseDto question =
        new DiagnosticQuestionResponseDto(
            questionId, "Question de stress ?", 50, true, LocalDateTime.now(), LocalDateTime.now());

    when(diagnosticService.listActiveQuestions()).thenReturn(List.of(question));

    mockMvc
        .perform(get("/api/v1/diagnostics/questions"))
  ...
  void calculateAnonymous_shouldReturnDiagnosticResult() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/v1/diagnostics/anonymous").contentType("application/json").content(body))
        .andExpect(status().isOk())
  ...
  void calculateAndSave_shouldForwardJwtAndReturnSavedResult() throws Exception {
    ...
    mockMvc
        .perform(post("/api/v1/diagnostics/submit").contentType("application/json").content(body))
        .andExpect(status().isOk())
  ...
  void myResults_shouldReturnCurrentUserResults() throws Exception {
    DiagnosticResultResponseDto result =
        new DiagnosticResultResponseDto(
            resultId, 150, "MODERE", "Stress modéré", LocalDateTime.now());

    when(diagnosticService.myResults(jwt)).thenReturn(List.of(result));

    mockMvc
        .perform(get("/api/v1/diagnostics/results/me"))
        .andExpect(status().isOk())
  ...
  void createQuestion_shouldReturnCreated() throws Exception {
    DiagnosticQuestionResponseDto response =
        new DiagnosticQuestionResponseDto(
            questionId, "Nouvelle question", 42, true, LocalDateTime.now(), LocalDateTime.now());
    ...
    mockMvc
        .perform(
            post("/api/v1/diagnostics/admin/questions")
  ...
  void updateQuestion_shouldCallService() throws Exception {
    DiagnosticQuestionResponseDto response =
        new DiagnosticQuestionResponseDto(
            questionId, "Question modifiée", 80, true, LocalDateTime.now(), LocalDateTime.now());
    ...
    mockMvc
        .perform(
            put("/api/v1/diagnostics/admin/questions/{id}", questionId)
                .contentType("application/json")
                .content(body))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.question").value("Question modifiée"));

    verify(diagnosticService)
        .updateQuestion(eq(questionId), any(DiagnosticQuestionRequestDto.class));
  }

  @Test
  void deleteQuestion_shouldReturnNoContent() throws Exception {
    mockMvc
        .perform(delete("/api/v1/diagnostics/admin/questions/{id}", questionId))
  ...
  void createResultConfig_shouldReturnCreated() throws Exception {
    DiagnosticResultConfigResponseDto response =
        new DiagnosticResultConfigResponseDto(
            resultConfigId,
            0,
            149,
            "FAIBLE",
    ...
    mockMvc
        .perform(
            post("/api/v1/diagnostics/admin/result-configs")
                .contentType("application/json")
                .content(body))
        .andExpect(status().isCreated())
  ...
  void deleteResultConfig_shouldReturnNoContent() throws Exception {
    mockMvc
        .perform(delete("/api/v1/diagnostics/admin/result-configs/{id}", resultConfigId))
        .andExpect(status().isNoContent());
