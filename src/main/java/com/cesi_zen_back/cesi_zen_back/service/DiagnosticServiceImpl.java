...
public class DiagnosticServiceImpl implements DiagnosticService {
  ...
  public List<DiagnosticQuestionResponseDto> listActiveQuestions() {
    return questionRepository.findByActiveTrueOrderByCreatedAtAsc().stream()
  ...
  public List<DiagnosticResultResponseDto> myResults(Jwt jwt) {
    ...
    return resultRepository.findByAppUserIdUserOrderByCreatedAtDesc(user.getIdUser()).stream()
  ...
  public List<DiagnosticQuestionResponseDto> listAdminQuestions() {
    return questionRepository.findAll().stream().map(DiagnosticMapper::toQuestionDto).toList();
  }
  ...
  public DiagnosticQuestionResponseDto updateQuestion(
      UUID id, DiagnosticQuestionRequestDto request) {
  ...
  public List<DiagnosticResultConfigResponseDto> listResultConfigs() {
    return resultConfigRepository.findAllByOrderByMinScoreAsc().stream()
  ...
  public DiagnosticResultConfigResponseDto updateResultConfig(
      UUID id, DiagnosticResultConfigRequestDto request) {
    validateResultConfig(request);
  ...
  private DiagnosticResponseDto calculateDiagnostic(
      DiagnosticRequestDto request, AppUser user, boolean saveResult) {
    List<DiagnosticQuestion> selectedQuestions =
        questionRepository.findAllById(request.questionIds());

    if (selectedQuestions.size() != request.questionIds().size()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Une ou plusieurs questions sont invalides");
    }
    ...
    return new DiagnosticResponseDto(resultId, finalScore, config.getLevel(), config.getMessage());
  }
  ...
  private DiagnosticQuestion getQuestion(UUID id) {
    return questionRepository
        .findById(id)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Question introuvable"));
  }
        ...
        .orElseThrow(
            () ->
                new ResponseStatusException(
                    HttpStatus.NOT_FOUND, "La configuration des résultats introuvable"));
  }
  ...
  private void validateResultConfig(DiagnosticResultConfigRequestDto request) {
    if (request.maxScore() < request.minScore()) {
      throw new ResponseStatusException(
          HttpStatus.BAD_REQUEST, "Le score maximum doit être supérieur ou égal au score minimum");
    }
  }

  private void applyResultConfigRequest(
      DiagnosticResultConfig config, DiagnosticResultConfigRequestDto request) {
