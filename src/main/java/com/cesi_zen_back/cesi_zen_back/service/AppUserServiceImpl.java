...
public class AppUserServiceImpl implements AppUserService {
  ...
  public UserDataExportDto exportCurrentUserData(String mail) {
    ...
    List<DiagnosticResultResponseDto> diagnosticResults =
        diagnosticResultRepository.findByAppUserIdUserOrderByCreatedAtDesc(userId).stream()
            .map(this::toDiagnosticResultDto)
            .toList();

    List<HistoricEtatResponseDto> histories =
        getUserHistories(userId).stream().map(HistoricEtatMapper::toDto).toList();

    List<AdminAuditLogExportDto> auditLogs =
        adminAuditLogRepository.findByTargetIdOrderByCreatedAtDesc(userId.toString()).stream()
  ...
  public AppUserDto promoteUser(UUID id, String adminMail) {
    return changeUserRole(
        id, adminMail, "ADMIN", "PROMOTE_USER", "Promotion au rôle ADMIN par un administrateur.");
  }
  ...
  private AppUserDto changeUserRole(
      UUID id, String adminMail, String roleName, String action, String details) {
    AppUser user = findUserById(id);
  ...
  private List<HistoricEtat> getUserHistories(UUID userId) {
    ...
    historicEtatRepository
        .findByAppUserIdUserOrderByModificationDateDesc(userId)
        .forEach(history -> histories.put(history.getId(), history));

    historicEtatRepository
        .findByEntityTypeAndEntityIdOrderByModificationDateDesc(TARGET_TYPE_USER, userId)
        .forEach(history -> histories.put(history.getId(), history));
